...
public class PaintShapes extends JPanel {
	...
	public static void showGUI(List<Rectangle> rectListParam, ConcurrentMap<String, String> intersectingMap,
			boolean isContain, String adjacentType) {
		...
		if (rectListParam != null && rectListParam.size() > 0) {
			...
			String text = "";
			if (isContain) {
				text += "CONTAINMENT - TRUE <br/>";
			} else {
				text += "NO CONTAINMENT <br/>";
			}
			for (String key : intersectingMap.keySet()) {
				if (key.equals(Constants.IS_INTERSECTS)) {
					if (intersectingMap.get(key).equalsIgnoreCase("TRUE")) {
						isIntersects = true;
						text += "INTERSECTION - TRUE <br/>";
					} else {
						text += "NO INTERSECTION <br/>";
					}
				}
				if (isIntersects == true && key.equals(Constants.INTERSECT_POINTS)) {
					text += "("+intersectingMap.get(key) + ") <br/>";
				}
			}
			text += adjacentType;
